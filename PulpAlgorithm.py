...
# mwf = [('MWF', 1), ('MWF', 2), ('MWF', 3), ('MWF', 4), ('MWF', 5), ('MWF', 6), ('MWF', 7), ('MWF', 8)]
# th = [('TTH', 1), ('TTH', 2), ('TTH', 3), ('TTH', 4), ('TTH', 5), ('TTH', 6), ('TTH', 7)]
# mw =  [('MW', 1), ('MW', 3), ('MW', 5), ('MW', 7)]
#Lets say a professor is only able to teach mwf, you can list their availbility to <'availability': mwf,>
#if they are only able to teach one time slot on one meeting pattern you can do availbility to <'availability': [('MWF', 1)],> or use manually scheduled classes
professors = {
    'Montek Singh': {
        'qualified_courses': ['COMP541', 'COMP572'],
        'availability': all_availability,  # Changed from time_slots to th
        'max_classes': 1
    },
    ...
    'Ketan Mayer-Patel': {
        ...
        'availability': all_availability,  # Changed from time_slots to mw
        'max_classes': 1
    },
    'Prairie Goodwin': {
    ...
    'Sayeed Ghani': {
        ...
        'availability': all_availability,  # Changed from all_availability to th
        'max_classes': 2
    },
    'P.S. Thiagarajan': {
    ...
    'Prasun Dewan': {
        ...
        'availability': all_availability,  # Already mwf, no change
        'max_classes': 1
    },
    'Jorge Silva': {
...
#5a ensures that a professor assignment (z) implies a real scheduled slot (x).
#5b ensures that a scheduled slot (x) implies at least one assigned, available professor (z).
#Together, these constraints prevent the scenario of assigning a professor without an actual class time, or scheduling a class time without assigning a professor.
...
#No MWF class right after a MW class in the same room, deprecated as Montek doesn't do that anyway
for r in rooms:
    for d in days:
...
#RUSh hour logic, deprecated, it crams into 8am,9am etc.
for mp in meeting_patterns:
...
if pulp.LpStatus[prob.status] == 'Optimal':
    ...
    for entry in schedule:
        ...
        print(f"  Days: {', '.join(entry['Days'])}, Period: {entry['Period']}, Start Time: {entry['Start Time']}")
...
#Remove and reassign helper method to take in a class, and reassign it to a new meeting pattern, time slot, and room with teh same professor.
