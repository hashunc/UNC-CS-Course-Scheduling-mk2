...
time_slot_mapping = {
    "MWF_1": "8:00 – 8:50 a.m.", "MWF_2": "9:05 – 9:55 a.m.", "MWF_3": "10:10 – 11:00 a.m.", "MWF_4": "11:15 – 12:05 p.m.",
    "MWF_5": "12:20 – 1:10 p.m.", "MWF_6": "1:25 – 2:15 p.m.", "MWF_7": "2:30 – 3:20 p.m.", "MWF_8": "3:35 – 4:25 p.m.",
    "MWF_9": "4:40 – 5:30 p.m.", "MWF_10": "5:45 – 6:35 p.m.",
    "MW_1": "8:00 – 9:15 a.m.", "MW_2": "9:30 – 10:45 a.m.", "MW_3": "11:00 – 12:15 p.m.", "MW_4": "12:30 – 1:45 p.m.",
    "MW_5": "2:00 – 3:15 p.m.", "MW_6": "3:30 – 4:45 p.m.", "MW_7": "5:00 – 6:15 p.m.",
    "TTH_1": "8:00 – 9:15 a.m.", "TTH_2": "9:30 – 10:45 a.m.", "TTH_3": "11:00 – 12:15 p.m.", "TTH_4": "12:30 – 1:45 p.m.",
    "TTH_5": "2:00 – 3:15 p.m.", "TTH_6": "3:30 – 4:45 p.m.", "TTH_7": "5:00 – 6:15 p.m."
}
...
def read_data(data_file):
    ...
    data['Professor_PreferredTimeSlots'] = data['Professor_PreferredTimeSlots'].apply(expand_time_slots)
...
def schedule_courses(data_file, output_file):
    ...
    time_slots = list(set(data['Professor_PreferredTimeSlots'].str.split(';').explode()))
    
    X = {(c, s, t): LpVariable(f"X_{c}_{s}_{t}", cat="Binary") for c, s in zip(data['CourseID'], data['Sec']) for t in time_slots}
    
    prob = LpProblem("Course_Scheduling", LpMinimize)
    prob += lpSum(X[c, s, t] for c, s in zip(data['CourseID'], data['Sec']) for t in time_slots)
    ...
    for professor in set(data['ProfessorName']):
        for t in time_slots:
            prob += lpSum(X[c, s, t] for c, s in zip(data['CourseID'], data['Sec']) if data.loc[(data['CourseID'] == c) & (data['Sec'] == s), 'ProfessorName'].values[0] == professor) <= 1
            
        # Constraint 4: Courses 301 and 211 cannot be scheduled at the same time
    for t in time_slots:
        prob += lpSum(X[c, s, t] for c, s in zip(data['CourseID'], data['Sec']) if c in [301, 211]) <= 1
    ...
    high_demand_periods = ["MWF_4", "MWF_5", "MWF_6", "MWF_7", "MW_3", "MW_4", "MW_5", "TTH_3", "TTH_4", "TTH_5"]
    total_courses = len(data)
    max_allowed = int(0.65 * total_courses)
    
    high_demand_courses = lpSum(X[c, s, t] for c, s in zip(data['CourseID'], data['Sec']) for t in time_slots if any(period in t for period in high_demand_periods))
    ...
    for c, s in zip(data['CourseID'], data['Sec']):
        for t in time_slots:
            if X[c, s, t].varValue == 1:
                professor_name = data.loc[(data['CourseID'] == c) & (data['Sec'] == s), 'ProfessorName'].values[0]
                section = s
                formatted_time = f"{t}: {time_slot_mapping.get(t, t)}"
                schedule.append([c, section, professor_name, formatted_time])
    
    schedule_df = pd.DataFrame(schedule, columns=["CourseID", "Sec", "ProfessorName", "Time"])
